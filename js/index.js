document.addEventListener("DOMContentLoaded", () => {
  //code for dashboardForecastCards content. City is declared and used to access data, max temp and wind speed are determined, and if/else determines the image displayed
  ...
  if (weatherCodeAmsterdam === 0) {
    ...
    (weatherCodeAmsterdam >= 51 && weatherCodeAmsterdam <= 67) ||
    (weatherCodeAmsterdam >= 80 && weatherCodeAmsterdam <= 82)
  ) {
    weatherAmsterdam = "/images/rain.png";
  } else if (weatherCodeAmsterdam >= 71 && weatherCodeAmsterdam <= 77) {
    weatherAmsterdam = "/images/snow.png";
    Cork;
  }
  const weatherTodayElementAmsterdam = weatherAmsterdam;

  const currentCityIndexBerlin = "berlin";
  const dailyDataIndexBerlin =
    weatherData[currentCityIndexBerlin + "_daily"].daily;
  const berlinDailyMaxTemp =
    dailyDataIndexBerlin.temperature_2m_max[0] + "&deg;C";
  const berlinDailyMaxWind =
    dailyDataIndexBerlin.wind_speed_10m_max[0] + "KM/H";
  ...
  if (weatherCodeBerlin === 0) {
    ...
    (weatherCodeBerlin >= 51 && weatherCodeBerlin <= 67) ||
    (weatherCodeBerlin >= 80 && weatherCodeBerlin <= 82)
  ) {
  ...
  const dailyDataIndexCopenhagen =
    weatherData[currentCityIndexCopenhagen + "_daily"].daily;
  const copenhagenDailyMaxTemp =
    dailyDataIndexCopenhagen.temperature_2m_max[0] + "&deg;C";
  const copenhagenDailyMaxWind =
    dailyDataIndexCopenhagen.wind_speed_10m_max[0] + "KM/H";
  ...
  if (weatherCodeCopenhagen === 0) {
    ...
    (weatherCodeCopenhagen >= 51 && weatherCodeCopenhagen <= 67) ||
    (weatherCodeCopenhagen >= 80 && weatherCodeCopenhagen <= 82)
  ) {
  ...
  if (weatherCodeCork === 0) {
    ...
    (weatherCodeCork >= 51 && weatherCodeCork <= 67) ||
    (weatherCodeCork >= 80 && weatherCodeCork <= 82)
  ) {
  ...
  const dailyDataIndexNewYork =
    weatherData[currentCityIndexNewYork + "_daily"].daily;
  const newYorkDailyMaxTemp =
    dailyDataIndexNewYork.temperature_2m_max[0] + "&deg;C";
  const newYorkDailyMaxWind =
    dailyDataIndexNewYork.wind_speed_10m_max[0] + "KM/H";
  ...
  if (weatherCodeNewYork === 0) {
    ...
    (weatherCodeNewYork >= 51 && weatherCodeNewYork <= 67) ||
    (weatherCodeNewYork >= 80 && weatherCodeNewYork <= 82)
  ) {
  ...
  const dailyDataIndexParis =
    weatherData[currentCityIndexParis + "_daily"].daily;
  const parisDailyMaxTemp =
    dailyDataIndexParis.temperature_2m_max[0] + "&deg;C";
  ...
  if (weatherCodeParis === 0) {
    ...
    (weatherCodeParis >= 51 && weatherCodeParis <= 67) ||
    (weatherCodeParis >= 80 && weatherCodeParis <= 82)
  ) {
  ...
  const dailyDataIndexSanFrancisco =
    weatherData[currentCityIndexSanFrancisco + "_daily"].daily;
  const sanFranciscoDailyMaxTemp =
    dailyDataIndexSanFrancisco.temperature_2m_max[0] + "&deg;C";
  const sanFranciscoDailyMaxWind =
    dailyDataIndexSanFrancisco.wind_speed_10m_max[0] + "KM/H";
  ...
  if (weatherCodeSanFrancisco === 0) {
    ...
    (weatherCodeSanFrancisco >= 51 && weatherCodeSanFrancisco <= 67) ||
    (weatherCodeSanFrancisco >= 80 && weatherCodeSanFrancisco <= 82)
  ) {
  ...
  const dailyDataIndexTromso =
    weatherData[currentCityIndexTromso + "_daily"].daily;
  const tromsoDailyMaxTemp =
    dailyDataIndexTromso.temperature_2m_max[0] + "&deg;C";
  const tromsoDailyMaxWind =
    dailyDataIndexTromso.wind_speed_10m_max[0] + "KM/H";
  ...
  if (weatherCodeTromso === 0) {
    ...
    (weatherCodeTromso >= 51 && weatherCodeTromso <= 67) ||
    (weatherCodeTromso >= 80 && weatherCodeTromso <= 82)
  ) {
  ...
  const dailyDataIndexWaterford =
    weatherData[currentCityIndexWaterford + "_daily"].daily;
  const waterfordDailyMaxTemp =
    dailyDataIndexWaterford.temperature_2m_max[0] + "&deg;C";
  const waterfordDailyMaxWind =
    dailyDataIndexWaterford.wind_speed_10m_max[0] + "KM/H";
  ...
  if (weatherCodeWaterford === 0) {
    ...
    (weatherCodeWaterford >= 51 && weatherCodeWaterford <= 67) ||
    (weatherCodeWaterford >= 80 && weatherCodeWaterford <= 82)
  ) {
  ...
  dashboardForecastCards.forEach(
    (dashboardForecastCard) =>
      (main.innerHTML += createDashbordForecastCardItem(dashboardForecastCard))
  );
//not used
//   const dataStoreDiv = document.querySelector("dataStore");
//   dataStore.list().forEach((titleForecastCard) => {
//     dataStoreDiv.innerHTML =
//       dataStoreDiv.innerHTML + createTitleForecastCardItem(titleForecastCard);
//   });
});
